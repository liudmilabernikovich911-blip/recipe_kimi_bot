...
class RecipeService:
    ...
    providers: List[DeepSeekProvider | OpenRouterProvider]
    ...
    async def get_recipes(self, system_prompt: str, user_prompt: str) -> str:
        ...
        for provider in self.providers:
            ...
            try:
                logger.info("Пробуем провайдер: %s", name)
                result = await provider.fetch(system_prompt, user_prompt)
                logger.info("Успех через %s", name)
                return result
            except Exception as exc:
                logger.warning("Ошибка в %s: %s", name, exc)
                last_error = exc
